  ...
  const startMigration = useCallback(async () => {
    ...
    try {
        ...
        const isDuplicate = existingCloudEntries.some(cloudEntry => {
          ...
          return (
            ...
            (cloudEntry.content &&
              ...
              cloudEntry.content.substring(0, 100) ===
      ...
      for (let i = 0; i < entriesToMigrate.length; i++) {
        ...
        try {
          setMigrationStatus(
            `Migrating entry ${i + 1} of ${entriesToMigrate.length}: "${entry.title || 'Untitled'}"`
          );
          ...
          result.errors.push(
            `Error migrating "${entry.title || 'Untitled'}": ${errorMessage}`
          );
      ...
      if (result.errors.length > 0) {
        ...
        setMigrationStatus(
          `Migration completed successfully! Migrated ${result.migratedCount} entries`
        );
      }
