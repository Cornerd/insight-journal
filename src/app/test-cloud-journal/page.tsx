...
export default function TestCloudJournalPage() {
  ...
  const handleCreateEntry = async () => {
    ...
    if (entry) {
      setNewTitle('');
      setNewContent('');
      showSuccess('Entry Created', `"${newTitle}" has been created successfully.`);
    } else {
      showError('Create Failed', 'Failed to create the entry. Please try again.');
    }
  };
  ...
  const handleUpdateEntry = async (id: string) => {
    ...
    if (entry) {
      ...
      setEditTitle('');
      setEditContent('');
      showSuccess('Entry Updated', `"${editTitle}" has been updated successfully.`);
    } else {
      showError('Update Failed', 'Failed to update the entry. Please try again.');
    }
  };
  ...
  const handleCreateAIAnalysis = async (entryId: string) => {
    ...
    if (analysis) {
      showSuccess('AI Analysis Created', 'AI analysis has been generated successfully.');
    } else {
      showError('Analysis Failed', 'Failed to create AI analysis. Please try again.');
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow-md">
          ...
          <p className="mb-4 text-gray-600">Please sign in to test cloud journal functionality.</p>
          <button
            onClick={() => signIn()}
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-green-50 p-4 rounded">
              ...
              <p className="text-2xl font-bold text-green-600">{entriesWithAnalysis.filter(e => e.summary).length}</p>
            </div>
            ...
            <div key={entry.id} className="bg-white rounded-lg shadow-md p-6">
              {editingId === entry.id ? (
                ...
                <div>
                  ...
                  {entriesWithAnalysis.find(e => e.id === entry.id && e.summary) && (
                    <div className="mt-4 p-3 bg-blue-50 rounded">
                      <h4 className="font-semibold text-blue-800 mb-2">AI Analysis</h4>
                      <p className="text-blue-700 text-sm">
                        {entriesWithAnalysis.find(e => e.id === entry.id)?.summary}
                      </p>
                    </div>
                  )}
                </div>
        ...
        {entries.length === 0 && !isLoading && (
          <div className="bg-white rounded-lg shadow-md p-6 text-center">
            <p className="text-gray-500">No journal entries yet. Create your first entry above!</p>
          </div>
        )}
      </div>
