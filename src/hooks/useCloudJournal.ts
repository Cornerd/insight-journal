...
interface UseCloudJournalReturn {
  ...
  updateEntry: (id: string, updates: { title?: string; content?: string }) => Promise<any | null>;
  deleteEntry: (id: string) => Promise<boolean>;
  refreshEntries: () => Promise<void>;
  createAIAnalysis: (entryId: string, summary: string, emotions: Record<string, unknown>, suggestions: Record<string, unknown>, model: string) => Promise<AIAnalysis | null>;
  clearError: () => void;
}
...
export function useCloudJournal(): UseCloudJournalReturn {
  ...
  const [entriesWithAnalysis, setEntriesWithAnalysis] = useState<JournalEntryWithAnalysis[]>([]);
  ...
  const refreshEntries = useCallback(async () => {
    ...
    try {
      ...
      if (entriesResponse.success) {
        ...
        const convertedEntries = (entriesResponse.data || []).map(convertCloudToLocalEntry);
        setEntries(convertedEntries);
      }

      // Load entries with analysis
      const analysisResponse = await apiCall('/api/journal/entries-with-analysis');
  ...
  // Update entry
  const updateEntry = useCallback(async (id: string, updates: { title?: string; content?: string }): Promise<JournalEntry | null> => {
    if (status !== 'authenticated') {
      setError('Not authenticated');
      return null;
    }
  ...
  // Create AI analysis
  const createAIAnalysis = useCallback(async (
    entryId: string,
    summary: string,
    emotions: Record<string, unknown>,
    suggestions: Record<string, unknown>,
    ...
    try {
      const response = await apiCall('/api/journal/ai-analysis', {
        method: 'POST',
        body: JSON.stringify({ entryId, summary, emotions, suggestions, model }),
      });
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to create AI analysis';
      setError(errorMessage);
