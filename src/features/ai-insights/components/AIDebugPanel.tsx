    ...
    <div className='fixed bottom-4 right-4 bg-gray-900 text-white p-4 rounded-lg shadow-lg max-w-md text-xs z-50'>
      ...
      <div className='space-y-2'>
        ...
        <div>
          ...
          {analysis ? '✅ Has Analysis' : '❌ No Analysis'}
        </div>
        ...
        {analysis && (
          <div className='mt-2 p-2 bg-gray-800 rounded'>
            ...
            <div>
              <strong>Emotions:</strong> {analysis.emotions?.length || 0}
            </div>
            <div>
              <strong>Suggestions:</strong> {analysis.suggestions?.length || 0}
            </div>
            <div>
        ...
        {currentEntry?.aiAnalysis && (
          <div className='mt-2 p-2 bg-green-800 rounded'>
            <div>
              <strong>Cached Analysis:</strong> ✅ Available
            </div>
            <div>
              <strong>Cached Type:</strong> {currentEntry.aiAnalysis.type}
            </div>
            <div>
              <strong>Cached Model:</strong> {currentEntry.aiAnalysis.model}
            </div>
          </div>
        )}
